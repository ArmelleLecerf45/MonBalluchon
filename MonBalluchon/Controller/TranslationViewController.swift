...
class TranslationViewController: UIViewController, UITextFieldDelegate {
    ...
    let targetLanguages = ["Anglais", "Français","Allemand", "Espagnol", "Italien", "Russe", "Grec", "Portugais"]
    ...
    @IBOutlet weak var ForeignLangageChoice: UIPickerView!
    ...
    private func translateButtonTaped() {
        ...
        let index = ForeignLangageChoice.selectedRow(inComponent: 0)

        TranslationService.shared.getTranslation(languageIndex: index , textToTranslate: text) { (true, translatedResponse) in
            self.toggleActivityIndicator(shown: false)
            self.translatedText.text = translatedResponse?.data.translations[0].translatedText
        }
    }
